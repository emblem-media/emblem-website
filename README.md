...
│   ├── form.js         採用フォームの送信処理
│   └── polish.js       演出系JS（スクロール表示アニメ・Navのフロスト化・スクロール進捗バー等）
└── assets/
    ├── images/         ロゴ・写真等
    └── videos/         飛行試験動画（mp4）
```
##　大前提
自分でこのREADMEを読んで編集するより、claude codeでこのフォルダを同期して編集してもらった方が圧倒的に楽です。
...
（コピーすれば、演出に必要な記述＝head の `js` クラス付与・`polish.js` 読み込み・各要素の `data-reveal`、および本文下部の「← News 一覧に戻る」ボタンも自動で引き継がれる。）

> 補足：本文に開催概要などで改行を入れたい場合は、その `<p>` に `data-html="true"` を付け、`data-jp` / `data-en` の中で `<br>` を使う（`news/nagoya-2026.html` が実例）。外部リンクを足す場合は `news-detail__link` の `<a>` を追加する。
...
**画像がロゴの場合**（白背景・切り抜きたくない）は、写真用クラスの代わりに contain 表示用クラスを使う:
- news.html: `news-card-full__img--photo` → `news-card-full__img--white`
- index.html（Step 3）: `hnj__update-thumb` → `hnj__update-thumb hnj__update-thumb--contain`

（実例：`news/nagoya-2026.html` の出展ロゴ）
...
## CSS / JS を編集したら（キャッシュ対策・重要）

`css/style.css` などを編集したら、**全HTMLの `?v=NN` の番号を1つ上げる**こと。
番号を上げないと、ブラウザやCDNが古いファイルをキャッシュしたままになり、変更が反映されない。

- 現在のCSS番号は `style.css?v=77`。次の編集で `v=78` のように +1 する。
- JSも同様：`js/main.js?v=`、`js/polish.js?v=`、`js/tech.js?v=` を編集したら番号を上げる。

対象は全HTML（index.html, technology.html, news.html, recruit.html, mission.html, team.html, privacy.html, news/ 以下の全ファイル）。一括置換で更新する。

```bash
# 例：CSSを v=77 → v=78 に一括置換（対象ファイルの確認）
grep -rl 'style.css?v=77' . --include='*.html'
```
