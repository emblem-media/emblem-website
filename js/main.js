...
(function initTechArchive() {
  function calcPattern(n) {
    ...
    if (rem === 0) {
      // すべて3列
      for (let i = 0; i < n / 3; i++) rows.push(3);
    } else if (rem === 2) {
      // 末尾1行だけ2枚
      const full = Math.floor(n / 3);
      for (let i = 0; i < full; i++) rows.push(3);
      rows.push(2);
    } else {
      // rem === 1
      if (n === 1) {
        rows.push(2); // 1枚は2列行の左寄せで表示
      } else {
        // 末尾4枚を 2+2 に（n-4 は必ず3の倍数）
        const full = (n - 4) / 3;
        for (let i = 0; i < full; i++) rows.push(3);
        rows.push(2);
        rows.push(2);
  ...
  function buildArchiveGrid() {
    ...
    if (!container) return;
    const items = Array.from(container.querySelectorAll('.tech-arc-item'));
    if (items.length === 0) return;

    const pattern = calcPattern(items.length);
    container.innerHTML = '';
    let cursor = 0;
    pattern.forEach(cols => {
      const row = document.createElement('div');
      row.className = `tech-arc-row tech-arc-row--${cols}`;
      for (let j = 0; j < cols && cursor < items.length; j++) {
  ...
  document.addEventListener('DOMContentLoaded', buildArchiveGrid);
})();
